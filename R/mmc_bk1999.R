#' @title mmc_bk1999
#' @description It calculates multi-market contacts with Baum & Korn's (1999) implementation.
#'
...
#' @param matrix_m A supplement data to indicate the revenue of firms in markets. Ideally It should have the same dimension and order as the data.
...
#'
#' \item market_cols.assets: The proportion of assets the focal firm (firm i) has across the markets.
#'
#' \item market_cols.y.assets: The proportion of assets the secondary firm (firm j) has across the markets.
#'
#' \item C_in+C_jn: The sum of the proportions of the assets that the focal firm (firm i) and the secondary firm (firm j) in the specific market.
#'
...
#'
#' \item sum_In_plus_sum_Jn: The sum of N_i and N_j. It is identical to SUM_n I_in + SUM_n I_jn.
#' }
#'
...
#' data = rmmc:::data
#' m = rmmc:::m
#' mmc_bk1999(data, m, "id", "year", "dyad", c("m1", "m2", "m3", "m4"))
mmc_bk1999 = function(data, matrix_m, firm_col, date_col = NA, level, market_cols = NA, subset_row = FALSE, fill_na = FALSE, full_result = FALSE,
  ...
  if (level == "dyad"){
    if (isTRUE(write_to_disk)){
      dir.create(paste0(save_path, "/bk1999"))
      ...
      for (i in 1:length(slider_start)){
        ...
        sum_In = sum_Jn = sum_In_plus_sum_Jn = N_i = N_j = NULL
        mmc_ij[, sum_In := rowSums(.SD), .SDcols = market]
        mmc_ij[, sum_Jn := rowSums(.SD), .SDcols = market_y]
        mmc_ij[, sum_In_plus_sum_Jn := sum_In + sum_Jn]
        mmc_ij[, N_i := rowSums(.SD), .SDcols = market]
        mmc_ij[, N_j := rowSums(.SD), .SDcols = market_y]

        sum_scale = NULL
        matrix_p[,  sum_scale := rowSums(.SD), .SDcols = market]
        matrix_p[, (market) := lapply(.SD, "/", sum_scale), .SDcols = market]
        matrix_p[, sum_scale := NULL]

        if (!is.na(date_col)){
          matrix_p = merge(matrix_p[slider_start[i]:slider_end[i], ], matrix_p, all = TRUE, allow.cartesian = TRUE, by = c(date_col), suffixes = c("", ".y"))
        } else {
          matrix_p = cbind(fake_date, matrix_p)
          matrix_p = merge(matrix_p[slider_start[i]:slider_end[i], ], matrix_p, all = TRUE, allow.cartesian = TRUE, by = c("fake_date"), suffixes = c("", ".y"))
          matrix_p[, fake_date := NULL]
        }

        matrix_p = matrix_p[matrix_p[[firm_col]] != matrix_p[[paste0(firm_col, ".y")]]]

        c_in_plus_c_jn_name = paste0("C_in+C_jn ", market)
        matrix_p[, (c_in_plus_c_jn_name) := matrix_p[, (market), with = FALSE] + matrix_p[, (market_y), with = FALSE]]

        if (!is.na(date_col)){
          mmc_ij = merge(mmc_ij, matrix_p, by = c(date_col, firm_col, paste0(firm_col, ".y")), suffixes = c("", ".asset"))
        } else {
          mmc_ij = merge(mmc_ij, matrix_p, by = c(firm_col, paste0(firm_col, ".y")), suffixes = c("", ".asset"))
        }

        mmc_ij[, mmc_ij := rowSums(mmc_ij[, (c_in_plus_c_jn_name), with = FALSE] * mmc_ij[, (IxI_col), with = FALSE])]
        mmc_ij[, mmc_ij := mmc_ij / (sum_In_plus_sum_Jn)]

        if (isFALSE(full_result)){
          mmc_ij[, c(market, market_y, IxI_col, "sum_In", "sum_Jn", "sum_In_plus_sum_Jn",
                     paste0(market, ".asset"), paste0(market, ".y.asset"), c_in_plus_c_jn_name) := NULL]
        }

        if (i == 1){
          if (isTRUE(full_result) & !is.na(date_col)){
            fwrite(mmc_ij[, c(date_col, firm_col, market, "N_i", paste0(firm_col, ".y"), market_y, "N_j",
                              paste0(market, ".asset"), paste0(market_y, ".asset"), c_in_plus_c_jn_name,
                              IxI_col, "sum_In_plus_sum_Jn", "mmc_ij"),
                          with = FALSE],
                   paste0(save_path, "/bk1999", "/", "mmc_ij.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isTRUE(full_result) & is.na(date_col)){
            fwrite(mmc_ij[, c(firm_col, market, "N_i", paste0(firm_col, ".y"), market_y, "N_j",
                              paste0(market, ".asset"), paste0(market_y, ".asset"), c_in_plus_c_jn_name,
                              IxI_col, "sum_In_plus_sum_Jn", "mmc_ij"),
                          with = FALSE],
                   paste0(save_path, "/bk1999", "/", "mmc_ij.gz"),
      ...
      if (!is.na(date_col)){
        matrix_p = merge(matrix_p, matrix_p, all = TRUE, allow.cartesian = TRUE, by = c(date_col), suffixes = c("", ".y"))
      } else {
        matrix_p = cbind(fake_date, matrix_p)
        matrix_p = merge(matrix_p, matrix_p, all = TRUE, allow.cartesian = TRUE, by = c("fake_date"), suffixes = c("", ".y"))
        matrix_p[, fake_date := NULL]
      }
      matrix_p = matrix_p[matrix_p[[firm_col]] != matrix_p[[paste0(firm_col, ".y")]]]
      ...
      if (isTRUE(full_result) & !is.na(date_col)){
        return(mmc_ij[, c(date_col, firm_col, market, "N_i", paste0(firm_col, ".y"), market_y, "N_j",
                          paste0(market, ".asset"), paste0(market_y, ".asset"), c_in_plus_c_jn_name,
                          IxI_col, "sum_In_plus_sum_Jn", "mmc_ij"),
                      with = FALSE])
      } else if (isTRUE(full_result) & is.na(date_col)){
        return(mmc_ij[, c(firm_col, market, "N_i", paste0(firm_col, ".y"), market_y, "N_j",
                          paste0(market, ".asset"), paste0(market_y, ".asset"), c_in_plus_c_jn_name,
                          IxI_col, "sum_In_plus_sum_Jn", "mmc_ij"),
                      with = FALSE])
      } else if (isFALSE(full_result) & !is.na(date_col)){
  ...
  if (level == "dyad market"){
    if (isTRUE(write_to_disk)){
      ...
      for (i in 1:length(slider_start)){
        ...
        mmc_ij[, c("sum_In", "sum_Jn", "sum_In_plus_sum_Jn",
                   paste0(market, ".asset"), paste0(market, ".y.asset"), c_in_plus_c_jn_name) := NULL]

        if (isFALSE(full_result)){
          mmc_ij[, c(IxI_col) := NULL]
        }
        ...
        if (i == 1){
          if (isTRUE(full_result) & !is.na(date_col)){
            fwrite(mmc_ij[, c(date_col, firm_col, market, "N_i", paste0(firm_col, ".y"), market_y, "N_j",
                   ...
                   paste0(save_path, "/bk1999", "/", "mmc_ijm.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE)
          } else if (isTRUE(full_result) & is.na(date_col)){
            fwrite(mmc_ij[, c(firm_col, market, "N_i", paste0(firm_col, ".y"), market_y, "N_j",
                              IxI_col, "mmc_ij", mmc_ijm_name, sum_mmc_ijm_name, avg_mmc_ijm_name), with = FALSE],
  ...
  if (level == "firm market"){
    if (isTRUE(write_to_disk)){
      ...
      for (i in 1:length(slider_start)){
        ...
        mmc_ij[, mmc_ij := rowSums(mmc_ij[, (c_in_plus_c_jn_name), with = FALSE] * mmc_ij[, (IxI_col), with = FALSE])]
        mmc_ij[, mmc_ij := mmc_ij / (sum_In_plus_sum_Jn)]
        mmc_ij[, c(c_in_plus_c_jn_name, "sum_In_plus_sum_Jn") := NULL]

        # calculate dyad-market
        mmc_ijm_name = paste("mmc_ij", market)
        mmc_ij[, (mmc_ijm_name) := lapply(.SD, function(x) x * mmc_ij), .SDcols = IxI_col]
        mmc_ij[, c(IxI_col, "mmc_ij") := NULL]
        ...
        if (i == 1){
          if (isTRUE(full_result) & !is.na(date_col)){
            fwrite(mmc_im[, c(date_col, firm_col, "N_i", mmc_ijm_name, n_mkt_name,
                   ...
                   paste0(save_path, "/bk1999", "/", "mmc_im.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isTRUE(full_result) & is.na(date_col)){
  ...
  if (level == "market"){
    if (isTRUE(write_to_disk)){
        ...
        if (i == 1){
          if (isTRUE(full_result) & !is.na(date_col)){
            fwrite(mmc_m[, c(date_col, mmc_im_name, n_mkt_name, mmc_m_name), with = FALSE],
                   paste0(save_path, "/bk1999", "/", "mmc_m.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isTRUE(full_result) & is.na(date_col)){
      ...
      mmc_m = fread(paste0(save_path, "/bk1999", "/", "mmc_m.gz"))
      ...
      fwrite(mmc_m, paste0(save_path, "/bk1999", "/", "mmc_m.gz"),
             append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE)
    ...
    if (isTRUE(write_to_disk)){
      ...
      if (isTRUE(full_result) & !is.na(date_col)){
        fwrite(mmc_mi[, c(date_col, firm_col, market, "N_i", n_mkt_name, mmc_m_name, mmc_mi_name, sum_mmc_mi_name, avg_mmc_mi_name), with = FALSE],
               paste0(save_path, "/bk1999", "/", "mmc_mi.gz"),
               append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE)
      } else if (isTRUE(full_result) & is.na(date_col)){
      ...
      if (isTRUE(full_result) & !is.na(date_col)){
        ...
        return(mmc_mi[, c(date_col, firm_col, mmc_mi_name, sum_mmc_mi_name, avg_mmc_mi_name), with = FALSE])
      } else if (isFALSE(full_result) & is.na(date_col)){
