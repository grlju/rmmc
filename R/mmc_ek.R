#' @title mmc_ekcount
#' @description It calculates multi-market contacts with Evans & Kessides' (1994): count implementation.
#'
...
mmc_ekcount = function(data, firm_col, date_col = NA, level, market_cols = NA, subset_row = FALSE, fill_na = FALSE, full_result = FALSE,
  ...
  if (level == "dyad"){
    if (isTRUE(write_to_disk)){
      dir.create(paste0(save_path, "/ek"))
      ...
      for (i in 1:length(slider_start)){
        ...
        mmc_ij[, mmc_ij := rowSums(.SD, na.rm = TRUE), .SDcols = IxI_col]
        ...
        if (isFALSE(full_result)){
          mmc_ij[, c(market, market_y, IxI_col) := NULL]
        }
  ...
  if (level == "dyad market"){
    if (isTRUE(write_to_disk)){
      ...
      for (i in 1:length(slider_start)){
        ...
        if (isFALSE(full_result)){
          mmc_ij[, c(market, market_y, IxI_col, "mmc_ij") := NULL]
        }
        ...
        if (i == 1){
          if (isTRUE(full_result) & !is.na(date_col)){
            fwrite(mmc_ij[, c(date_col, firm_col, market, "N_i", paste0(firm_col, ".y"), market_y, "N_j",
                   ...
                   paste0(save_path, "/ek", "/", "mmc_ijm.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isTRUE(full_result) & is.na(date_col)){
  ...
  if (level == "market"){
    if (isTRUE(write_to_disk)){
      ...
      mmc_m = fread(paste0(save_path, "/ek", "/", "mmc_m.gz"))
      ...
      fwrite(mmc_m, paste0(save_path, "/ek", "/", "mmc_m.gz"),
             append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE)
