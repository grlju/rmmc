#' @title mmc_bk
#' @description It calculates multi-market contacts with Baum & Korn's (1996) implementation.
#'
...
#'
#' \item Sum In * Jn: The row sum of the IxI columns.
#'
#' \item Sum In: The number of markets in which the focal firm (firm i) is present.
#' }
#'
...
#' @examples
#' data = rmmc:::data
#' mmc_bk(data, "id", "year", "market", market_cols = c("m1", "m2", "m3", "m4"))
mmc_bk = function(data, firm_col, date_col = NA, level, market_cols = NA, subset_row = FALSE, fill_na = FALSE, full_result = FALSE,
  ...
  if (level == "dyad"){
    if (isTRUE(write_to_disk)){
      dir.create(paste0(save_path, "/bk"))
      ...
      for (i in 1:length(slider_start)){
        ...
        mmc_ij[, "Sum In * Jn" := rowSums(.SD, na.rm = TRUE), .SDcols = IxI_col]
        mmc_ij[, "Sum In" := rowSums(.SD, na.rm = TRUE), .SDcols = market]
        mmc_ij[, mmc_ij := mmc_ij[, "Sum In * Jn"] / mmc_ij[, "Sum In"]]
        mmc_ij[, mmc_ij := ifelse(is.nan(mmc_ij), 0, mmc_ij)]
        ...
        if (i == 1){
          if (isTRUE(full_result) & !is.na(date_col)){
            fwrite(mmc_ij[, c(date_col, firm_col, "N_i", market,
                              paste0(firm_col, ".y"), "N_j", market_y,
                              IxI_col, "Sum In * Jn", "Sum In", "mmc_ij"), with = FALSE],
                   paste0(save_path, "/bk", "/", "mmc_ij.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isTRUE(full_result) & is.na(date_col)){
            fwrite(mmc_ij[, c(firm_col, "N_i", market,
                              paste0(firm_col, ".y"), "N_j", market_y,
                              IxI_col, "Sum In * Jn", "Sum In", "mmc_ij"), with = FALSE],
                   paste0(save_path, "/bk", "/", "mmc_ij.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
      ...
      if (isTRUE(full_result) & !is.na(date_col)){
        return(mmc_ij[, c(date_col, firm_col, "N_i", market,
                          paste0(firm_col, ".y"), "N_j", market_y,
                          IxI_col, "Sum In * Jn", "Sum In", "mmc_ij"), with = FALSE])
      } else if (isTRUE(full_result) & is.na(date_col)){
        return(mmc_ij[, c(firm_col, "N_i", market,
                          paste0(firm_col, ".y"), "N_j", market_y,
                          IxI_col, "Sum In * Jn", "Sum In", "mmc_ij"), with = FALSE])
      } else if (isFALSE(full_result) & !is.na(date_col)){
  ...
  if (level == "dyad market"){
    if (isTRUE(write_to_disk)){
      ...
      for (i in 1:length(slider_start)){
        ...
        mmc_ij[, (mmc_ijm_name) := lapply(.SD, function(x) x * mmc_ij), .SDcols = IxI_col]

        sum_mmc_ijm_name = "Sum mmc_ijm"
        mmc_ij[, (sum_mmc_ijm_name) := rowSums(.SD, na.rm = TRUE), .SDcols = mmc_ijm_name]
        non_zero_entries_name = "Non zero"
        mmc_ij[, (non_zero_entries_name) := rowSums(.SD != 0), .SDcols = mmc_ijm_name]
        avg_mmc_ijm_name = "Avg mmc_ijm"
        ans = (mmc_ij[, sum_mmc_ijm_name, with = FALSE]) / (mmc_ij[, non_zero_entries_name, with = FALSE])
        ans = ifelse(is.nan(ans[[1]]), 0, ans[[1]])
        mmc_ij[, (avg_mmc_ijm_name) := ans]

        if (i == 1){
          if (isTRUE(full_result) & !is.na(date_col)){
            fwrite(mmc_ij[, c(date_col, firm_col, market, "N_i",
                              paste0(firm_col, ".y"), market_y, "N_j",
                              IxI_col, "Sum In * Jn", "Sum In", "mmc_ij", mmc_ijm_name, sum_mmc_ijm_name, avg_mmc_ijm_name), with = FALSE],
                   paste0(save_path, "/bk", "/", "mmc_ijm.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isTRUE(full_result) & is.na(date_col)){
            fwrite(mmc_ij[, c(firm_col, "N_i", market,
                              paste0(firm_col, ".y"), "N_j", market_y,
                              IxI_col, "mmc_ij", mmc_ijm_name, sum_mmc_ijm_name, avg_mmc_ijm_name), with = FALSE],
                   paste0(save_path, "/bk", "/", "mmc_ijm.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isFALSE(full_result) & !is.na(date_col)){
            fwrite(mmc_ij[, c(date_col, firm_col, "N_i", paste0(firm_col, ".y"), "N_j",
                              mmc_ijm_name, sum_mmc_ijm_name, avg_mmc_ijm_name), with = FALSE],
                   paste0(save_path, "/bk", "/", "mmc_ijm.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE)
          } else if (isFALSE(full_result) & is.na(date_col)){
            fwrite(mmc_ij[, c(firm_col, "N_i", paste0(firm_col, ".y"), "N_j", mmc_ijm_name,
                              sum_mmc_ijm_name, avg_mmc_ijm_name), with = FALSE],
                   paste0(save_path, "/bk", "/", "mmc_ijm.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE)
          }
        } else {
      ...
      if (isTRUE(full_result) & !is.na(date_col)){
        ...
        return(mmc_ij[, c(date_col, firm_col, "N_i", paste0(firm_col, ".y"), "N_j",
                          mmc_ijm_name, sum_mmc_ijm_name, avg_mmc_ijm_name), with = FALSE])
      } else if (isFALSE(full_result) & is.na(date_col)){
        return(mmc_ij[, c(firm_col, "N_i", paste0(firm_col, ".y"), "N_j", mmc_ijm_name,
                          sum_mmc_ijm_name, avg_mmc_ijm_name), with = FALSE])
      }
    }
  }
  ...
  if (level == "firm market"){
    if (isTRUE(write_to_disk)){
      ...
      for (i in 1:length(slider_start)){
        ...
        if (!is.na(date_col)){
          mmc_im = mmc_ij[, lapply(.SD, sum), by = c(date_col, firm_col), .SDcols = mmc_ijm_name]
        } else {
          mmc_im = mmc_ij[, lapply(.SD, sum), by = c(firm_col), .SDcols = mmc_ijm_name]
        }

        b_ij = IxI_total = NULL
        mmc_ij[, IxI_total := rowSums(.SD, na.rm = TRUE), .SDcols = IxI_col]
        mmc_ij[, b_ij := lapply(.SD, function (x) ifelse(x > 1, 1, 0)), .SDcols = "IxI_total"]
        mmc_ij[, (mmc_ijm_name) := lapply(.SD, function(x) x * b_ij), .SDcols = mmc_ijm_name]

        # calculate the numerator
        if (!is.na(date_col)){
          mmc_im_numerator = mmc_ij[, lapply(.SD, sum), by = c(date_col, firm_col), .SDcols = mmc_ijm_name]
        } else {
          mmc_im_numerator = mmc_ij[, lapply(.SD, sum), by = c(firm_col), .SDcols = mmc_ijm_name]
        }
        mmc_im_numerator_name = paste("sum b_ij * mmc_ijm", mmc_ijm_name)
        setnames(mmc_im_numerator, mmc_ijm_name, mmc_im_numerator_name)

        # calculate the denominator
        if (!is.na(date_col)){
          mmc_im_denominator = mmc_ij[, lapply(.SD, function(x) sum(x * b_ij)), by = c(date_col, firm_col), .SDcols = market_y]
        } else {
          mmc_im_denominator = mmc_ij[, lapply(.SD, function(x) sum(x * b_ij)), by = c(firm_col), .SDcols = market_y]
        }
        mmc_im_denominator_name = paste("sum b_ij * I_jm", market)
        setnames(mmc_im_denominator, market_y, mmc_im_denominator_name)

        if (!is.na(date_col)){
          mmc_im = merge(mmc_im_numerator, mmc_im_denominator, by = c(date_col, firm_col))
        } else {
          mmc_im = merge(mmc_im_numerator, mmc_im_denominator, by = c(firm_col))
        }

        # calculate numerator/denominator
        mmc_im_name = paste("mmc_im", market)
        mmc_im[, (mmc_im_name) := mmc_im[, (mmc_im_numerator_name), , with = FALSE] /
                 mmc_im[, (mmc_im_denominator_name), , with = FALSE]]
        ...
        if (i == 1){
          if (isTRUE(full_result) & !is.na(date_col)){
            fwrite(mmc_im[, c(date_col, firm_col, "N_i", mmc_im_numerator_name, mmc_im_denominator_name, mmc_im_name, sum_mmc_im_name, avg_mmc_im_name), with = FALSE],
                   paste0(save_path, "/bk", "/", "mmc_im.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isTRUE(full_result) & is.na(date_col)){
            fwrite(mmc_im[, c(firm_col, "N_i", mmc_im_numerator_name, mmc_im_denominator_name, mmc_im_name, sum_mmc_im_name, avg_mmc_im_name), with = FALSE],
                   paste0(save_path, "/bk", "/", "mmc_im.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isFALSE(full_result) & !is.na(date_col)){
      ...
      if (isTRUE(full_result) & !is.na(date_col)){
        return(mmc_im[, c(date_col, firm_col, "N_i", mmc_im_numerator_name, mmc_im_denominator_name, mmc_im_name, sum_mmc_im_name, avg_mmc_im_name), with = FALSE])
      } else if (isTRUE(full_result) & is.na(date_col)){
        return(mmc_im[, c(firm_col, "N_i", mmc_im_numerator_name, mmc_im_denominator_name, mmc_im_name, sum_mmc_im_name, avg_mmc_im_name), with = FALSE])
      } else if (isFALSE(full_result) & !is.na(date_col)){
  ...
  if (level == "market"){
    if (isTRUE(write_to_disk)){
      ...
      for (i in 1:length(slider_start)){
        ...
        mmc_im_numerator_name = paste0("sum b_ij * mmc_ijm", mmc_ijm_name)
        setnames(mmc_im_numerator, mmc_ijm_name, mmc_im_numerator_name)
        ...
        mmc_im_denominator_name = paste0("sum b_ij * I_jm", market)
        setnames(mmc_im_denominator, market_y, mmc_im_denominator_name)
        ...
        # calculate numerator/denominator
        mmc_im_name = paste("mmc_i", market)
        mmc_im[, (mmc_im_name) := mmc_im[, (mmc_im_numerator_name), , with = FALSE] /
                 mmc_im[, (mmc_im_denominator_name), , with = FALSE]]

        mmc_im[, (mmc_im_name) := lapply(.SD, function(x) ifelse(is.nan(x), 0, x)), .SDcols = mmc_im_name]
        mmc_im[, c(mmc_im_numerator_name, mmc_im_denominator_name) := NULL]
      ...
      mmc_m = fread(paste0(save_path, "/bk", "/", "mmc_m.gz"))
      ...
      fwrite(mmc_m, paste0(save_path, "/bk", "/", "mmc_m.gz"),
             append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE)
  ...
  if (level == "market firm"){
    if (isTRUE(write_to_disk)){
      ...
      for (i in 1:length(slider_start)){
        ...
        if (i == 1){
          if (isTRUE(full_result) & !is.na(date_col)){
            fwrite(mmc_m[, c(date_col, mmc_im_name, n_mkt_name, mmc_m_name, sum_mmc_m_name, avg_mmc_m_name), with = FALSE],
                   paste0(save_path, "/bk", "/", "mmc_m.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isTRUE(full_result) & is.na(date_col)){
            fwrite(mmc_m[, c(mmc_im_name, n_mkt_name, mmc_m_name, sum_mmc_m_name, avg_mmc_m_name), with = FALSE],
                   paste0(save_path, "/bk", "/", "mmc_m.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isFALSE(full_result) & !is.na(date_col)){
            fwrite(mmc_m[, c(date_col, mmc_m_name, sum_mmc_m_name, avg_mmc_m_name), with = FALSE],
                   paste0(save_path, "/bk", "/", "mmc_m.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE)
          } else if (isFALSE(full_result) & is.na(date_col)){
            fwrite(mmc_m[, c(mmc_m_name, sum_mmc_m_name, avg_mmc_m_name), with = FALSE],
                   paste0(save_path, "/bk", "/", "mmc_m.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE)
          }
        } else {
