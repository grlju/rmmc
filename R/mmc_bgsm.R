#' @title mmc_bgsm
#' @description It calculates multi-market contacts with Boeker et al's (1997) implementation.
#'
...
#' @param fill_na Logical. True to fill NA in the data with 0.
...
#' @param chunk_size An integer. Size of chunks for processing and saving the data. Higher chunk size will increase memory usage but may shorten the time to write results to the disk.
...
#' @examples
#' data = rmmc:::data
#' mmc_bgsm(data, "id", "year", "market", market_cols = c("m1", "m2", "m3", "m4"))
mmc_bgsm = function(data, firm_col, date_col = NA, level, market_cols = NA, subset_row = FALSE, fill_na = FALSE, full_result = FALSE,
  ...
  if (level == "dyad"){
    if (isTRUE(write_to_disk)){
      dir.create(paste0(save_path, "/bgsm"))
      ...
      for (i in 1:length(slider_start)){
        ...
        if (i == 1){
          if (isTRUE(full_result) & !is.na(date_col)){
            fwrite(mmc_ij[, c(date_col, firm_col, market, "N_i", paste0(firm_col, ".y"), market_y, "N_j", IxI_col,
                              "Sum In * Jn", "Sum In", "mmc_ij"), with = FALSE],
                   paste0(save_path, "/bgsm", "/", "mmc_ij.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isTRUE(full_result) & is.na(date_col)){
            fwrite(mmc_ij[, c(firm_col, market, "N_i",
      ...
      if (isTRUE(full_result) & !is.na(date_col)){
        return(mmc_ij[, c(date_col, firm_col, market, "N_i",
                          paste0(firm_col, ".y"), market_y, "N_j",
                          IxI_col, "Sum In * Jn", "Sum In", "mmc_ij"), with = FALSE])
      } else if (isTRUE(full_result) & is.na(date_col)){
        return(mmc_ij[, c(firm_col, market, "N_i",
                          paste0(firm_col, ".y"), market_y, "N_j",
                          IxI_col, "Sum In * Jn", "Sum In", "mmc_ij"), with = FALSE])
      } else if (isFALSE(full_result) & !is.na(date_col)){
        ...
        if (i == 1){
          if (isTRUE(full_result) & !is.na(date_col)){
            fwrite(mmc_ij[, c(date_col, firm_col, market, "N_i",
                   ...
                   paste0(save_path, "/bgsm", "/", "mmc_ijm.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isTRUE(full_result) & is.na(date_col)){
            fwrite(mmc_ij[, c(firm_col, market, "N_i",
  ...
  if (level == "firm market"){
    if (isTRUE(write_to_disk)){
      ...
      for (i in 1:length(slider_start)){
        ...
        mmc_im[, (mmc_im_name) := lapply(.SD, function(x) ifelse((is.nan(x) | is.infinite(x)), 0, x)), .SDcols = mmc_im_name]
        ...
        if (i == 1){
          if (isTRUE(full_result) & !is.na(date_col)){
            fwrite(mmc_im[, c(date_col, firm_col, "N_i", mmc_ijm_name, n_mkt_name, mmc_im_name, sum_mmc_im_name, avg_mmc_im_name), with = FALSE],
                   paste0(save_path, "/bgsm", "/", "mmc_im.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isTRUE(full_result) & is.na(date_col)){
            fwrite(mmc_im[, c(firm_col, "N_i", mmc_ijm_name, n_mkt_name, mmc_im_name, sum_mmc_im_name, avg_mmc_im_name), with = FALSE],
                   paste0(save_path, "/bgsm", "/", "mmc_im.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isFALSE(full_result) & !is.na(date_col)){
      ...
      if (isTRUE(full_result) & !is.na(date_col)){
        return(mmc_im[, c(date_col, firm_col, "N_i", mmc_ijm_name, n_mkt_name, mmc_im_name, sum_mmc_im_name, avg_mmc_im_name), with = FALSE])
      } else if (isTRUE(full_result) & is.na(date_col)){
        return(mmc_im[, c(firm_col, "N_i", mmc_ijm_name, n_mkt_name, mmc_im_name, sum_mmc_im_name, avg_mmc_im_name), with = FALSE])
      } else if (isFALSE(full_result) & !is.na(date_col)){
  ...
  if (level == "market"){
    if (isTRUE(write_to_disk)){
        ...
        if (i == 1){
          if (isTRUE(full_result) & !is.na(date_col)){
            fwrite(mmc_m[, c(date_col, mmc_im_name, n_mkt_name, mmc_m_name), with = FALSE],
                   paste0(save_path, "/bgsm", "/", "mmc_m.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isTRUE(full_result) & is.na(date_col)){
      ...
      mmc_m = fread(paste0(save_path, "/bgsm", "/", "mmc_m.gz"))
      ...
      fwrite(mmc_m, paste0(save_path, "/bgsm", "/", "mmc_m.gz"),
             append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE)
    ...
    if (isTRUE(write_to_disk)){
      ...
      if (isTRUE(full_result) & !is.na(date_col)){
        fwrite(mmc_mi[, c(date_col, firm_col, market, "N_i", n_mkt_name, mmc_m_name, mmc_mi_name, sum_mmc_mi_name, avg_mmc_mi_name), with = FALSE],
               paste0(save_path, "/bgsm", "/", "mmc_mi.gz"),
               append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE)
      } else if (isTRUE(full_result) & is.na(date_col)){
