#' @title mmc_chen
#'
#' @description It calculates multi-market contacts with Chen's (1999) implementation.
#'
...
#'
#' \item market_cols.assets: The proportion of assets the focal firm (firm i) has across the markets. (P_in)
#'
#' \item market_cols.y.assets: The proportion of assets the secondary firm (firm j) has across the markets. (P_jn)
#'
#' \item P_in/P_i m: The number of assets of the focal firm in a market divided by the total number of asset of the focal firm across the markets.
#'
#' \item P_jn/P_n m: The number of assets of the secondary firm in a market divided by the total number of asset of the secondary firm across the markets.
#'
#' \item PxP m: (P_in/P_i) * (P_jn/P_n) in their respective market
#' }
#'
...
#' data = rmmc:::data
#' m = rmmc:::m
#' mmc_chen(data, m, "id", "year", "dyad", c("m1", "m2", "m3", "m4"))
mmc_chen = function(data, matrix_m, firm_col, date_col = NA, level, market_cols = NA, subset_row = FALSE, fill_na = FALSE, full_result = FALSE,
  ...
  if (level == "dyad"){
    if (isTRUE(write_to_disk)){
      dir.create(paste0(save_path, "/chen"))
      ...
      for (i in 1:length(slider_start)){
        ...
        matrix_p[, P_i := rowSums(.SD), .SDcols = market]
        pin_pi_name = paste("P_in/P_i", market)
        matrix_p[, (pin_pi_name) := lapply(.SD, function(x) x / P_i), .SDcols = market]
        matrix_p[, P_n := sum(.SD), .SDcols = market_y]
        pjn_pn_name = paste("P_jn/P_n", market_y)
        matrix_p[, (pjn_pn_name) := lapply(.SD, function(x) x / P_n), .SDcols = market_y]
        PxP_name = paste("PxP", market)
        matrix_p[, (PxP_name) := matrix_p[, (pin_pi_name), with = FALSE] * matrix_p[, (pjn_pn_name), with = FALSE]]
        matrix_p[, mmc_ij := rowSums(.SD), .SDcols = PxP_name]

        if (isFALSE(full_result)){
          matrix_p[, c(market, market_y, "P_i", pin_pi_name, "P_n", pjn_pn_name, PxP_name) := NULL]
        }

        if (i == 1){
          if (isTRUE(full_result) & !is.na(date_col)){
            fwrite(matrix_p[, c(date_col, firm_col, market, "P_i", paste0(firm_col, ".y"),
                                market_y, "P_n", pin_pi_name, pjn_pn_name, PxP_name, "mmc_ij"), with = FALSE],
                   paste0(save_path, "/chen", "/", "mmc_ij.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isTRUE(full_result) & is.na(date_col)){
            fwrite(matrix_p[, c(firm_col, market, "P_i", paste0(firm_col, ".y"),
                                market_y, "P_n", pin_pi_name, pjn_pn_name, PxP_name, "mmc_ij"), with = FALSE],
                   paste0(save_path, "/chen", "/", "mmc_ij.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isFALSE(full_result) & !is.na(date_col)){
            fwrite(matrix_p[, c(date_col, firm_col, paste0(firm_col, ".y"), "mmc_ij"), with = FALSE],
                   paste0(save_path, "/chen", "/", "mmc_ij.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE)
          } else if (isFALSE(full_result) & is.na(date_col)){
            fwrite(matrix_p[, c(firm_col, paste0(firm_col, ".y"), "mmc_ij"), with = FALSE],
                   paste0(save_path, "/chen", "/", "mmc_ij.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE)
          }
        } else {
      ...
      if (isTRUE(full_result) & !is.na(date_col)){
        return(matrix_p[, c(date_col, firm_col, market, "P_i", paste0(firm_col, ".y"),
                            market_y, "P_n", pin_pi_name, pjn_pn_name, PxP_name, "mmc_ij"), with = FALSE])
      } else if (isTRUE(full_result) & is.na(date_col)){
        return(matrix_p[, c(firm_col, market, "P_i", paste0(firm_col, ".y"),
                            market_y, "P_n", pin_pi_name, pjn_pn_name, PxP_name, "mmc_ij"), with = FALSE])
      } else if (isFALSE(full_result) & !is.na(date_col)){
        return(matrix_p[, c(date_col, firm_col, paste0(firm_col, ".y"), "mmc_ij"), with = FALSE])
      } else if (isFALSE(full_result) & is.na(date_col)){
        return(matrix_p[, c(firm_col, paste0(firm_col, ".y"), "mmc_ij"), with = FALSE])
      }
    }
  }
  ...
  if (level == "dyad market"){
    if (isTRUE(write_to_disk)){
      ...
      for (i in 1:length(slider_start)){
        ...
        matrix_p = matrix_p[matrix_p[[firm_col]] != matrix_p[[paste0(firm_col, ".y")]]]
        matrix_p[, P_i := rowSums(.SD), .SDcols = market]
        matrix_p[, (market) := lapply(.SD, function(x) x / P_i), .SDcols = market]
        matrix_p[, P_n := sum(.SD), .SDcols = market_y]
        matrix_p[, (market_y) := lapply(.SD, function(x) x / P_n), .SDcols = market_y]
        PxP_name = paste("mmc_ij", market)
        matrix_p[, (PxP_name) := matrix_p[, (market), with = FALSE] * matrix_p[, (market_y), with = FALSE]]
        matrix_p[, mmc_ij := rowSums(.SD), .SDcols = PxP_name]
        matrix_p[, c(market, market_y, "P_i", "P_n", PxP_name) := NULL]

        # calculate dyad-market
        if (!is.na(date_col)){
          mmc_ijm = merge(mmc_ij, matrix_p, by = c(date_col, firm_col, paste0(firm_col, ".y")))
        } else {
          mmc_ijm = merge(mmc_ij, matrix_p, by = c(firm_col, paste0(firm_col, ".y")))
        }

        mmc_ijm_name = paste("mmc_ij", market)
        mmc_ijm[, (mmc_ijm_name) := lapply(.SD, function(x) x * mmc_ij), .SDcols = IxI_col]

        if (isFALSE(full_result)){
          mmc_ijm[, c(IxI_col, "mmc_ij") := NULL]
        }
        ...
        if (i == 1){
          if (isTRUE(full_result) & !is.na(date_col)){
            fwrite(mmc_ijm[, c(date_col, firm_col, market, "N_i", paste0(firm_col, ".y"), market_y, "N_j", IxI_col, "mmc_ij",
                               mmc_ijm_name, sum_mmc_ijm_name, avg_mmc_ijm_name), with = FALSE],
                   paste0(save_path, "/chen", "/", "mmc_ijm.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isTRUE(full_result) & is.na(date_col)){
            fwrite(mmc_ijm[, c(firm_col, market, "N_i", paste0(firm_col, ".y"), market_y, "N_j", IxI_col, "mmc_ij",
                               mmc_ijm_name, sum_mmc_ijm_name, avg_mmc_ijm_name), with = FALSE],
                   paste0(save_path, "/chen", "/", "mmc_ijm.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
      ...
      if (isTRUE(full_result) & !is.na(date_col)){
        return(mmc_ijm[, c(date_col, firm_col, market, "N_i", paste0(firm_col, ".y"), market_y, "N_j", IxI_col, "mmc_ij",
                           mmc_ijm_name, sum_mmc_ijm_name, avg_mmc_ijm_name), with = FALSE])
      } else if (isTRUE(full_result) & is.na(date_col)){
        return(mmc_ijm[, c(firm_col, market, "N_i", paste0(firm_col, ".y"), market_y, "N_j", IxI_col, "mmc_ij",
                           mmc_ijm_name, sum_mmc_ijm_name, avg_mmc_ijm_name), with = FALSE])
      } else if (isFALSE(full_result) & !is.na(date_col)){
        ...
        if (i == 1){
          if (isTRUE(full_result) & !is.na(date_col)){
            fwrite(mmc_im[, c(date_col, firm_col, "N_i", mmc_ijm_name, n_mkt_name,
                   ...
                   paste0(save_path, "/chen", "/", "mmc_im.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isTRUE(full_result) & is.na(date_col)){
  ...
  if (level == "market"){
    if (isTRUE(write_to_disk)){
        ...
        if (i == 1){
          if (isTRUE(full_result) & !is.na(date_col)){
            fwrite(mmc_m[, c(date_col, mmc_im_name, n_mkt_name, mmc_m_name), with = FALSE],
                   paste0(save_path, "/chen", "/", "mmc_m.gz"),
                   append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE
            )
          } else if (isTRUE(full_result) & is.na(date_col)){
      ...
      mmc_m = fread(paste0(save_path, "/chen", "/", "mmc_m.gz"))
      ...
      fwrite(mmc_m, paste0(save_path, "/chen", "/", "mmc_m.gz"),
             append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE)
    ...
    if (isTRUE(write_to_disk)){
      ...
      if (isTRUE(full_result) & !is.na(date_col)){
        fwrite(mmc_mi[, c(date_col, firm_col, market, "N_i", n_mkt_name, mmc_m_name, mmc_mi_name, sum_mmc_mi_name, avg_mmc_mi_name), with = FALSE],
               paste0(save_path, "/chen", "/", "mmc_mi.gz"),
               append = FALSE, col.names = TRUE, row.names = FALSE, compress = "auto", showProgress = TRUE)
      } else if (isTRUE(full_result) & is.na(date_col)){
